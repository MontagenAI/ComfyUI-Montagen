...
class FFCreator extends FFCon {
  constructor(conf = {}) {
    ...
    if (!isBrowser) {
      FFLogger.info({
        pos: 'OpenCV',
        msg: `enable: ${OpenCVUtil.enable} available: ${OpenCVUtil.available()}`,
      });
    }
  }
  ...
  async resize(width, height) {
    if (
      ...
      isNaN(width) ||
      isNaN(height) ||
      (width === this.getConf('width') && height === this.getConf('height'))
    )
      return;
    // 这三组更新都是必须的。。。ft!
    this.width = width;
  ...
  generateOutput() {
    ...
    const output = this.getConf('output');
    if (outputDir && !output) {
  ...
  xml(json, indent, tag = null) {
    ...
    const attrs = [];
    const children =
      json.children && Array.isArray(json.children)
