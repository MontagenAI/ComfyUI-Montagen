...
class GifMaterial extends VideoMaterial {
  async init(opts) {
    ...
    this.imageData = [];
    for (let i = 0; i < this.frameInfo.length; i++) {
      let disposal = this.frameInfo[i].disposal >= 2 ? 3 : 1;
      ...
      if (disposal === 1) {
        ...
        this.drawCanvas(imageData[i], width, height, canvas);
        if (!isBrowser) this.imageData.push(ctx.getImageData(0, 0, width, height));
